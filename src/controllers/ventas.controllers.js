...
export const createVenta = async (req, res) => {
	...
	if (stripe.activo === true) {
		let urlStripe = await pagosStripe((aPagar),stripe.accessToken,moneda,stripe.porcentaje,req.params.tienda);
...
const pagosStripe=async (monto,privateKey,moneda,costo,tienda) => {
	...
	diviza=diviza[0];
		...
		const session = await stripe.checkout.sessions.create({				
		  ...
		  success_url: `https://tienda.delivery/${tienda}/pago_ok`,
    	  cancel_url: `https://tienda.delivery/${tienda}/error_de_pago`,
		  mode: 'payment'
		});
		 return session.url;
	  };
